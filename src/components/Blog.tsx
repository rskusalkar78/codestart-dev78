...
const ResourceCard: React.FC<ResourceCardProps> = ({
  ...
  return <Card className="bg-portfolio-lightNavy border-portfolio-lightestNavy h-full portfolio-card hover:shadow-lg transition-shadow duration-300">
      <CardContent className="p-6 flex flex-col h-full bg-zinc-950">
        ...
        <Button asChild variant="link" className="text-portfolio-green hover:text-portfolio-green/90 p-0 self-start mt-auto">
...
const Blog: React.FC = () => {
  ...
  return <section id="blog">
      <h2 className="text-2xl md:text-3xl font-bold section-title numbered-heading">
        <span className="text-2xl font-normal">04.</span> Resources for Beginners
      </h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {beginnerResources.map((resource, index) => <ResourceCard key={index} {...resource} />)}
      </div>
